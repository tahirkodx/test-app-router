...
import FindGameNav from "@/components/FPS/FindGameNav";
...
const FindGamingPCsByFPS = () => {
  ...
  useEffect(() => {
    if (params !== null && params !== undefined) {
      let gameTtl = params.gameTitile;
      let resol = params.resolutions;
  ...
  useEffect(() => {
    ...
    const fetchPageData = async () => {
      const pdatas = await ProductAPI.gamingPCsByFps();
      ...
      if (prods.length > 0) {
        let filProds = _.filter(
          _.map(prods, (product: any) => {
            ...
            let selectedGameData = _.first(
              _.filter(gameData, (game: any) => {
                return game.titles === gameTitle;
              })
            );
            if (selectedGameData !== undefined || selectedGameData !== null) {
          ...
          (product: any) => {
            return product.FPSScore?.trim().length > 0;
          }
        );
                        ...
                        max !== undefined && (
                          <>
                            <DoubleRangeSlider
                              ...
                              // Remove the 'title' prop from the component
                              onMouseUp={({ min, max }) => {
                                setMinRangeUp(min);
                                setMaxRangeUp(max);
                              }}
...
export default FindGamingPCsByFPS;
