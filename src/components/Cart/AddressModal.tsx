...
const AddressModal = (props: any) => {
  ...
  const submitForm = async (event: any) => {
    ...
    let add: any = data.get("txtAddress");
    let addressId: any = data.get("txtAddressID");
    let country: any = data.get("drpCountry");
    let state: any = data.get("drpStates");
    let city: any = data.get("drpCity");
    let postcode: any = data.get("txtPostCode");
    let suburb: any = data.get("txtSuburb");
    let contact: any = data.get("txtContactPerson");
    let chkPrimary: any = data.get("chkIsPrimary");
    let isPrimary: any = chkPrimary ? true : false;
    if (
      ![add, country, state, city, postcode, suburb, contact].some(
        (str: any) => str.trim().length === 0
      )
    ) {
  ...
  useEffect(() => {
    const getState = async () => {
      ...
      /*    const summary = await fetch(`/api/util/getState`, {
        method: "GET",
        headers: {
          Authorization: "Bearer " + authCtx.token,
          "Content-Type": "application/json",
